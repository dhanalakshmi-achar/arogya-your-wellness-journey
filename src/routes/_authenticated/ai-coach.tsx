...
import { createFileRoute } from "@tanstack/react-router";
import { Sparkles, Send, RotateCcw, Mic, Loader2 } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { useApp } from "@/store/app";
import { coachReply } from "@/lib/ai";
import { toast } from "sonner";
...
export const Route = createFileRoute("/_authenticated/ai-coach")({
  head: () => ({
    meta: [
      { title: "AI Coach — Arogya" },
      { name: "description", content: "Chat with your personal AI health coach." },
    ],
  }),
  component: AICoach,
});
...
function AICoach() {
  ...
  const send = async (text: string) => {
    ...
    setLoading(true);
    const history = useApp.getState().chat;
    addChat({ role: "user", text: trimmed, at: Date.now() });
    setInput("");

    try {
      const state = useApp.getState();
      const reply = await coachReply(trimmed, state, history);
      addChat({ role: "ai", text: reply, at: Date.now() });
    } catch (err) {
      console.error(err);
      toast.error("AI Coach is having trouble. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="mx-auto flex h-[calc(100dvh-6rem)] max-w-3xl flex-col px-4 py-6 md:px-8 md:py-10">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={clearChat}
          className="inline-flex items-center gap-1 rounded-2xl border bg-card px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground"
        >
          <RotateCcw className="h-3 w-3" /> New chat
        </button>
      </div>
      ...
      <div
        ref={scrollRef}
        className="mt-6 flex-1 space-y-3 overflow-y-auto rounded-3xl border bg-card p-4 shadow-soft"
      >
        {chat.map((m) => (
          <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div
      ...
      <div className="mt-3 flex flex-wrap gap-1">
        {PROMPTS.map((p) => (
          <button
            ...
            onClick={() => send(p)}
            disabled={loading}
            className="rounded-full border bg-background px-3 py-1 text-xs text-muted-foreground hover:border-primary hover:text-primary disabled:opacity-50"
          >
            {p}
          </button>
        ))}
      ...
      <form
        ...
        className="mt-3 flex items-center gap-2 rounded-full border bg-card p-2 shadow-soft"
      >
        <button
          type="button"
          onClick={startVoice}
          className={`grid h-10 w-10 place-items-center rounded-full ${
            listening ? "bg-destructive text-white" : "bg-muted text-muted-foreground"
          }`}
          aria-label="Voice"
        >
          <Mic className="h-4 w-4" />
        ...
        <button
          ...
          className="grid h-10 w-10 place-items-center rounded-full bg-primary text-primary-foreground shadow-glow disabled:opacity-50"
          aria-label="Send"
        >
          <Send className="h-4 w-4" />
        </button>
